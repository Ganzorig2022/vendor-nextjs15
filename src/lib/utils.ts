import {
	...
	District,
  IBank
} from "@/modules/general/types/type";
...
export const getCityName = (value: string) => {
	...
	if (typeof window !== "undefined") {
		...
		if (vendorStore) {
			try {
				const parsedStore = JSON.parse(vendorStore);
				CITY_ARRAY = parsedStore?.state?.generalData?.cities || [];
			} catch (error) {
				console.error("Error parsing vendor-store:", error);
				CITY_ARRAY = [];
			}
...
export const getBankName = (value: string) => {
	let BANKS;
	if (typeof window !== "undefined") {
		const vendorStore = sessionStorage.getItem("vendor-store");
		if (vendorStore) {
			try {
				const parsedStore = JSON.parse(vendorStore);
				BANKS = parsedStore?.state?.generalData?.banks || [];
			} catch (error) {
				console.error("Error parsing vendor-store:", error);
				BANKS = [];
			}
